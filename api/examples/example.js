/*
*IMPORTANT:
*    API is shared between GUI and the pure API, and acts as the backbone of the GUI part.
*    However, in order to use the standalone API in a programming environment and run this demo,
*    tradebridgecoin-api, rather than the api within tradebridgecoin-gui, has to be used!
*/
...
//the infura entry point used below is just for trial. Any production environment entails a dedicated entry point that can be obtained by signing up infura account
...
let account = ''; // specify the account; different accounts in main net vs. test net
...
let toOrderSet = new Set();
let stage = 10; //set the starting stage: 0 - deposit; 1 - withdraw; 2 - transfer TBC; 3 - make orders
...
TD.newInstance({endpoint, account}).then( async td => {
    ...
    //deposit 0.123456 ether
    setInterval(async () => {
        if (stage === 0) {
            stage = -1;
            console.log('\ndeposit 0.123456 ether...this will take tens of seconds to minutes');
            td
    ...
    //subscribe to market data and make orders
    setInterval(() => {
        if(stage === 3){
            stage = -1;
            ...
            md
                ...
                .subscribe([
                    ...
                    {category: 2, symbols:['USD.CNY.FX', 'EUR.CNY.FX', 'USD.JPY.FX']},
                    {category: 3, symbols:['IVX.SS']},
                    {category: 4, symbols:['TBC.CNY.CRYPTO', 'ETH.USD.CRYPTO', 'BTC.JPY.CRYPTO', 'EOS.TBC.CRYPTO']},
                    {category: 5, symbols:['RB.CNC', 'CU.CNC']}
                ]);
            ...
            let quoteHandle = async (c, f) => {
                ...
                if(!toOrderSet.has(f.symbol)) {
                    toOrderSet.add(f.symbol);

                    if (md.isInTradingHours(c, f)) {
                        let orderJSON = {
                        ...
                        console.log(`${f.symbol} ${f.moment} is not in the trading hours!`)
                    }
                }
                md.unsubscribe([{category: c, quote: f}])
            };
            ...
            md.onMarketData(quote =>{
                if(quote.error)
                    ...
                    quote.quotes.forEach(f => quoteHandle(quote.category, f))
            });
        }
    }, 500);
