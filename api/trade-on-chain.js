...
const appDataDir = process.env.HOME;
...
class Trade {
    ...
    static newInstance({endpoint, account, agent = '', cache = true, pendingNum = 0}){
        ...
        web3.eth.getTransactionCount(account)
            .then(n => {
                if(cache){
                    let db = new sqlite.Database(`${appDataDir}/tbc_cache_${endpoint.hashCode()}.db`, () => {
                        try{
    ...
    order(args, errFunc){
        ...
        let c1 = category.toString() === '1' && !new RegExp(meta.categories['1'].symbols[0]).test(symbol) ||
                 category.toString() === '5' && !new RegExp(meta.categories['5'].symbols[0]).test(symbol);
        if(c1 || category.toString() !== '1' && category.toString() !== '5'  && (meta.supportedCategories.indexOf(category.toString()) < 0 || meta.categories[category.toString()].symbols.indexOf(symbol) < 0)){
            if(errFunc !== undefined)
            ...
            md.onSnapshot(quote => {
                switch(quote.category){
                        ...
                        quote.quotes.forEach(f => {
                            positions.filter(ff => ff.trade.symbol === f.symbol).forEach(async ffe => {
                                ...
                                if(histRor === undefined || histRor.length === 0){
                                    ...
                                    if(histRor.length === 0)
                                        histRor = await util.getOHLCFromYahooV8(f.symbol);
                                    this.histRoR.set(f.symbol, histRor)
                                }
                                if(histRor !== undefined){
                                    ...
                                    fe.ror = fe.direction * (histRor.length === 0 ? f.price / fe.cost - 1 : ((1 + f.percent) * cumRoR / (1 + fe.firstRoR) - 1)) * fe.leverage;
                                    fe.preROR = histRor.length === 0 ? f.price / fe.cost - 1 : cumRoR - 1;
                    ...
                    case 4:
                        ...
                        positions.filter(a =>a.trade.marketValue === undefined).forEach(f => console.log(JSON.stringify(f)));
                        if(positions.reduce((r, a) => {r = r && (a.trade.marketValue !== undefined); return r}, true)){
                            let pos = positions.map(m => {
                                m.trade.tokenInvested = Number((m.trade.tokenInvested / 1e6).toFixed(6));
                                m.trade.txHash = m.transactionHash;
                        ...
                        break;
                    case 5:
                        console.log(` - prices of category 5 positions ${(positions.filter(f => f.trade.category === 5) || []).length} retrieved...`);
                        quote.quotes.forEach(f => {
                            positions.filter(ff => ff.trade.symbol === f.symbol).forEach(ffe => {
                                let fe = ffe.trade;
                                fe.currentPrice = f.quote;
                                fe.ror = fe.direction * (fe.currentPrice / fe.cost- 1) * fe.leverage;
                                fe.marketValue = Number((fe.tokenInvested * (1 + fe.ror) / 1e6).toFixed(6));
                                fe.time = moment(fe.timestamp).format('YYYY-MM-DD HH:mm:ss')
                            });
                        ...
                        subs.filter(f => f.category === 5).map(m => m.symbols).flatten().filter(f => !quote.quotes.some(s => s.symbol === f)).forEach(fe => {
                           if(Number(fe.substring(2, 6)) <= Number(moment.tz('Asia/Shanghai').format('YYMM'))){
                               positions.filter(ff => ff.trade.symbol === fe).forEach(ffe => {
                                   let fe = ffe.trade;
                                   fe.currentPrice = 0;
                                   fe.ror = -1;
                                   fe.marketValue = 0;
