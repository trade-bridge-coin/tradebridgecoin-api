...
class IVX {
    ...
    async ivx() {
        ...
        switch (this.provider.toLowerCase()) {
            case 'sina_api': {
                Promise.all([this.shibor3m(), request('GET', 'http://hq.sinajs.cn/list=sh000300')]).then(result => {
                    result
                });
                ...
                let calls = new Map(await Promise.all(Array.from(callCodes, async m => [m[0], await Promise.all(Array.from(m[1],async mm => {//console.log(`call:\t${mm}`);
                        let rsp = (await request('GET', quoteUrl + mm)).body.toString('utf-8').split('=')[1].replace(/"/g, '').replace(';\n','').split(',');
                        return this.rspToObj(rsp, 1, m[0]);
                    }
                ))])));
                let puts = new Map(await Promise.all(Array.from((putCodes), async m => [m[0], await Promise.all(Array.from(m[1], async mm => {//console.log(`put:\t${mm}`);
                        let rsp = (await request('GET', quoteUrl + mm)).body.toString('utf-8').split('=')[1].replace(/"/g, '').replace(';\n','').split(',');
